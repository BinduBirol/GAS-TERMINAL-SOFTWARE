...
public class DefaulterCertificate extends ActionSupport implements
		ServletContextAware {
	...
	public String execute() throws Exception {
		...
		if (download_type.equals("individual_wise")) { // ////// Individual
														// Report generate korle
														// History te Data
														// felano jabe na
			...
			try {
				if (report_for.equals("area_wise")) {
					...
					if (download_type.equals("individual_wise")) {
						clearnessDTO = getCustomerInfo(customer_id, area);
						dueMonthList = getDueMonth(customer_id);
					} else {

						clearnessDTO = getCustomerInfo(customerList.get(a).getCustomerID(), area);
						dueMonthList = getDueMonth(customerList.get(a)
								.getCustomerID());
					}
					...
					if (listSize == 0) {
						...
						if (collection_month == null && calender_year == null) {
							...
							if (print_type.equals("01")) {
								realPath = servlet
										.getRealPath("/resources/staticPdf/CCertificate.pdf");
							} else if (print_type.equals("02")) {
								realPath = servlet
										.getRealPath("/resources/staticPdf/Printable.pdf");
							}

							// realPath =
							// servlet.getRealPath("/resources/staticPdf/CCertificate.pdf");
							...
							BaseFont bf = BaseFont.createFont(
									BaseFont.TIMES_ROMAN, BaseFont.WINANSI,
									BaseFont.EMBEDDED);
							...
							over.showText(Month.values()[Integer.valueOf(month) - 2]
									+ "," + year);
							...
							if (clearnessDTO.getFathersName().equalsIgnoreCase(
									"N/A")
									|| clearnessDTO.getFathersName() == null) {
												...
												new Chunk(
														...
														FontFactory
																.getFont(
																		...
																		8,
																		Font.NORMAL))),
										70, 65, 380, 50, 15, Element.ALIGN_LEFT
												| Element.ALIGN_TOP);
								ct1.go();
							} else {
								ColumnText ct = new ColumnText(over);
							...
							over.showText("Revenue Section : "
									+ String.valueOf(Area.values()[Integer
											.valueOf(loggedInUser.getArea_id()) - 1]));
							...
							over.setFontAndSize(bf, 9);
							over.setTextMatrix(73, 689);
							over.showText(Month.values()[Integer
									.valueOf(collection_month) - 1]
									+ ","
									+ calender_year);
							...
							stamp.close();
							readers.add(new PdfReader(certificate.toByteArray()));
							insertClarificationHistory(customerList.get(a)
									.getCustomerID(), from_date,
									loggedInUser.getUserId(), 1); // 1=No Dues
						}
					}
					...
					if (listSize != 0) {
						...
						if (collection_month == null && calender_year == null) {
							...
							realPath = servlet
									.getRealPath("/resources/staticPdf/ClearnessCertificate.pdf");
							document = new Document();
							out = new ByteArrayOutputStream();
							// left,right,top,bottom
							fileName = "ClearnessCertificate.pdf";
							...
							for (int i = 0; i < listSize; i++) {
								duesString = duesString + ","
										+ dueMonthList.get(i).getDueMonth();

								totaldueAmount = totaldueAmount
										+ dueMonthList.get(i).getDueAmount();
								...
								if (i == listSize - 1) {
									...
									over.showText(Month.values()[Integer
											.valueOf(month) - 2] + "," + year);

									double bqty = clearnessDTO.getBurner();

									String burnerQty = Double.toString(bqty);

									String areaId = clearnessDTO
											.getCustomerID();
									String area = areaId.substring(0, 2);

									over.setFontAndSize(bf, 11);
									over.setTextMatrix(310, 610);
									over.showText(String.valueOf(clearnessDTO
											.getBurner()));

									over.setFontAndSize(bf, 11);
									over.setTextMatrix(426, 610);
									over.showText(String.valueOf(taka_format
											.format(totaldueAmount)));
									// over.showText("ABCDEFG");

									String areaName = String
											.valueOf(Area.values()[Integer
													.valueOf(area) - 1]);
									...
									over.showText(String.valueOf(Area.values()[Integer
											.valueOf(area) - 1]));
									...
									image.setAnnotation(new Annotation(0, 0, 0,
											0, 3));
									over.addImage(image);
							...
							stamp.close();
							readers.add(new PdfReader(certificate.toByteArray()));
							// insertClarificationHistory(customerList.get(a).getCustomerID(),from_date,loggedInUser.getUserId(),0);
							// // 0= Dues
						} else {
							...
							// realPath =
							// servlet.getRealPath("/resources/staticPdf/ClearnessCertificate.pdf");
							document = new Document();
							out = new ByteArrayOutputStream();
							// left,right,top,bottom
							fileName = "ClearnessCertificate.pdf";
							...
							for (int i = 0; i < listSize; i++) {
								...
								if (i == listSize - 1) {
									...
									if (area.equals("02") || area.equals("06")
											|| area.equals("07")
											|| area.equals("08")
											|| area.equals("09")) {
										areaName = "BAGHABARI";
									...
									ct.setSimpleColumn(
											...
											Element.ALIGN_LEFT
													| Element.ALIGN_TOP);
									ct.go();
									if (clearnessDTO.getFathersName()
							...
							insertClarificationHistory(customerList.get(a)
									.getCustomerID(), from_date,
									loggedInUser.getUserId(), 0); // 0= Dues
						}
		...
		try {
			if (collection_month == null) {
				...
				String transaction_sql = "SELECT customer_id, BILL_ID, BILL_MONTH, BILL_YEAR, TO_CHAR (TO_DATE (BILL_MONTH, 'MM'), 'MON')||''''|| SUBSTR (BILL_YEAR, 3) MONTH_YEAR,PAYABLE_AMOUNT, "
						+ "0 PAID_AMOUNT, PAYABLE_AMOUNT DUE_AMOUNT  "
						+ "						FROM BILL_METERED  "
						+ "						WHERE status = 1 AND BILL_YEAR||lpad(bill_month,2,0) <= "
						+ calender_year
						+ billMonth
						+ " AND customer_id ='"
						+ customer_id
						...
						+ "						WHERE customer_id ='"
						+ customer_id
						+ "' AND STATUS = 1 AND BILL_YEAR||lpad(bill_month,2,0) <= "
						+ calender_year + billMonth + " ";
	...
	public ArrayList<ClearnessDTO> getDueMonthWeb(String customer_id) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				cDto.setBillId(resultSet.getString("BILL_ID")); // for online
																// bill payment
																// select
																// checkbox,
																// sujon
	...
	public ArrayList<ClearnessDTO> getDueMonthWeb(String customer_id,
			String billIds) {
		ArrayList<ClearnessDTO> dueMonth = new ArrayList<ClearnessDTO>();
		String billIdClause = billIds.equals("") ? "" : " and Bill_Id in ("
				+ billIds + ")";
		...
		try {
			String transaction_sql = "select customer_id, BILL_ID, BILL_MONTH, BILL_YEAR,MONTH_YEAR,PAYABLE_AMOUNT,PAID_AMOUNT,DUE_AMOUNT,round((PAYABLE_AMOUNT*12*daydiff)/(100*365),0) SURCHARGE from( "
					...
					+ "  FROM BILL_NON_METERED "
					+ " WHERE customer_id ='"
					+ customer_id + "' AND STATUS = 1  " + billIdClause;
	...
	private ClearnessDTO getCustomerInfo(String customer_id, String area_id) {
		...
		try {
			String customer_info_sql = 
					...
					"                 AND bi.area_id = ? " +
					"                 AND BI.CUSTOMER_ID = ? " +
					//"                 AND BILL_YEAR || LPAD (BILL_MONTH, 2, 0) <= 2017010 " +
			...
			/*
			 * 12/3/2018 12.50pm String customer_info_sql="SELECT CUSTOMER_ID, "
			 ...
			 * "       FATHER_NAME, " + "       ADDRESS_LINE1,PHONE, " +
			 * "       SDATE, " + "       NEXTDAY, " + "       PYEAR, " +
			 * "       NEW_DOUBLE_BURNER_QNT_BILLCAL, " + "       MAX_LOAD " +
			 ...
			 * "UNION ALL " + "SELECT CUSTOMER_ID, " + "       FULL_NAME, " +
			 * "       FATHER_NAME, " + "       ADDRESS_LINE1,PHONE, " +
			 * "       SDATE, " + "       NEXTDAY, " + "       PYEAR, " +
			 * "       nvl(NEW_DOUBLE_BURNER_QNT_BILLCAL,0) NEW_DOUBLE_BURNER_QNT_BILLCAL, "
			 * + "       MAX_LOAD " + "  FROM (SELECT CUSTOMER_ID, " +
			 * "               FULL_NAME, " + "               FATHER_NAME, " +
			...
			/*while (resultSet.next()) {
				...
				ctrInfo.setBurner(resultSet
						.getDouble("OLD_APPLIANCE_QNT_BILLCALL"));
			...
			while (resultSet.next()){
				...
				ctrInfo.setCustomerName(resultSet.getString("FULL_NAME"));
				ctrInfo.setCustomerAddress(resultSet.getString("ADDRESS_LINE1"));
				ctrInfo.setDueAmount(Double.parseDouble(resultSet.getString("TOTALAMOUNT")));
	...
	private ClearnessDTO getPayDate() {
		...
		try {
			String transaction_sql = "Select to_char(to_date('" + from_date
					+ "','dd-MM-YYYY')-30) PAYDATE from dual";
	...
	private ArrayList<ClearnessDTO> getCustomerList(String from_cus_id,
			String to_cus_id, String cus_type, String cust_cat_id, String area) {
		...
		try {
			String transaction_sql = "select CUSTOMER_ID,STATUS from  "
					+ "(select CUSTOMER_ID,to_number(substr(CUSTOMER_ID,1,9)) CUS_ID,getDuesStatus(CUSTOMER_ID,"
					+ calender_year
					+ ",ISMETERED) STATUS from MVIEW_CUSTOMER_INFO  "
					+ "Where substr(CUSTOMER_ID,1,2)=" + area
					+ "AND ISMETERED=" + cus_type + "AND CATEGORY_ID="
					+ cust_cat_id + ")" + "Where STATUS=1 and cus_id between "
					+ from_cus_id + " and " + to_cus_id
